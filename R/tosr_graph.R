...
graph.bib <- function(scopus_dataframe, ext){
  ...
  graph.info <- list(graph = graph_2,
                     node_cite = nodes_scopus_type,
                     ext = ext,
                     cited_ref = cited_references)
  return(graph.info)
}
...
graph_ml <- function(biblio_wos_scopus,ext){
  ...
  graph.info <- list(graph = graph,
                     node_cite = nodes_attributes,
                     ext = ext,
                     cited_ref = cited_references)
  return(graph.info)
}
